...
if [ -n "$status_file" ]; then
  ...
  if [ "$command_name" = "implement" ] && [ "$terminal_reason" = "completed" ]; then
    ...
    if [ "$proj_changed" = "yes" ] || [ "$rows_advanced" = "yes" ] || \
      ...
      error_msg="the step reported success but changed things outside analyze's scope — $scope_what. Press Run again for this step."
    fi
  fi
fi
