...
| Which archive refusals are guards, not attempts — left out of the phase's attempt count (2026-09-11)                                                        | the `case "$terminal_reason"` arm in `core/scripts/lib/run-spec-outcome.sh`      | `GUARD_REFUSALS` in `dashboard/src/render/pages/queue-list/phase-rows.ts`                                          | `dashboard/test/queue/guard-refusals-are-not-attempts.test.ts`, reading both sides' source text                                                                                                    |
