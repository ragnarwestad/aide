...
describe("the line a phone shows at 02:00", () => {
  test("a finished step says what is done, what it cost and where to look", async () => {
    const hook = webhook();
    try {
      expect(await run(FINISHED, { webhookUrl: hook.url })).toBe(0);
      expect(hook.posted.length).toBe(1);
      const text = hook.posted[0]!.text ?? "";
      expect(text).toContain("aide · 81-queue-and-runner");
      expect(text).toContain("analyze done");
      expect(text).toContain("$2.1");
      expect(text).toContain(FINISHED.branchUrl);
    } finally {
      await hook.stop();
    }
  });
...
describe("failing quietly", () => {
  ...
  test("an unreachable webhook is still exit 0", async () => {
    expect(await run(FINISHED, { webhookUrl: "http://127.0.0.1:9/hook" })).toBe(0);
  });
});
