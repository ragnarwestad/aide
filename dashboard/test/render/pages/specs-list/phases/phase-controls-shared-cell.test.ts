...
describe("spec 192: the phase line's controls share one cell", () => {
  const target = (specFolder = "192-one-cell"): SpecTarget => ({
    project: "aide",
    specFolder,
  });
  ...
  test("the merge leaves six columns, and the caption matches them", () => {
    const html = rows();
    // The caption line and every phase line write the same number, or
    // the table stops lining up with its own head row.
