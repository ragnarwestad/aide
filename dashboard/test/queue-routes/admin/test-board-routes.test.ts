...
import { MAIN_BOARD_KEY } from "../../../src/serve/boards/lifecycle.ts";
...
/** What `defaultBranch()`'s own `symbolic-ref` and `startBoard()`'s own
...
const mainEntry = (overrides: Partial<BoardEntry> = {}): BoardEntry => ({
  branch: "main",
...
describe("POST /api/queue/projects/<name>/test-board (AC-3, AC-5, AC-6)", () => {
  test("refuses for a project this dashboard does not know", async () => {
    const { base } = start({ queueToken: TOKEN });
    const res = await fetch(`${base}/api/queue/projects/nosuch/test-board`, { method: "POST", headers: AUTH });
    expect(res.status).toBe(404);
  });

  test("only POST — the route takes no other method", async () => {
    const { base } = start({ queueToken: TOKEN });
    const res = await fetch(`${base}/api/queue/projects/aide/test-board`, { headers: AUTH });
    expect(res.status).toBe(405);
  });

  test("the round being unavailable answers boardFailedPage", async () => {
    const git = onMain();
    const { base } = start({ queueToken: TOKEN, gitRun: git.run, boardsAvailable: false });
    const res = await fetch(`${base}/api/queue/projects/aide/test-board`, { method: "POST", headers: AUTH });
  ...
  test("success resolves the default branch, spawns on it, and 303s to the waiting URL", async () => {
    ...
    const res = await fetch(`${base}/api/queue/projects/aide/test-board`, {
      method: "POST",
      redirect: "manual",
      headers: AUTH,
    });
    expect(res.status).toBe(303);
    expect(res.headers.get("location")).toBe("/projects/aide?startTestBoard=1");
  ...
  test("a second press restarts rather than starting a second board", async () => {
    ...
    const first = await fetch(`${base}/api/queue/projects/aide/test-board`, {
      method: "POST",
      redirect: "manual",
      headers: AUTH,
    });
    expect(first.status).toBe(303);
    const second = await fetch(`${base}/api/queue/projects/aide/test-board`, {
      method: "POST",
      redirect: "manual",
      headers: AUTH,
    });
  ...
  test("cannot resolve the default branch: boardFailedPage, no spawn", async () => {
...
describe("POST /api/queue/projects/<name>/test-board/stop (AC-7)", () => {
  test("refuses for a project this dashboard does not know", async () => {
    const { base } = start({ queueToken: TOKEN });
    const res = await fetch(`${base}/api/queue/projects/nosuch/test-board/stop`, { method: "POST", headers: AUTH });
    expect(res.status).toBe(404);
  });

  test("only POST — the route takes no other method", async () => {
    const { base } = start({ queueToken: TOKEN });
    const res = await fetch(`${base}/api/queue/projects/aide/test-board/stop`, { headers: AUTH });
    expect(res.status).toBe(405);
  });
  ...
  test("stops a tracked main-board with no spec ref, redirecting for a no-script POST", async () => {
    ...
    server.boardsStore().set("aide", MAIN_BOARD_KEY, mainEntry());
    const res = await fetch(`${base}/api/queue/projects/aide/test-board/stop`, {
      method: "POST",
      redirect: "manual",
    ...
    expect(server.boardsStore().get("aide", MAIN_BOARD_KEY)).toBeUndefined();
  });
  ...
  test("answers JSON for the actionform's own XHR", async () => {
    const { base, server } = start({ queueToken: TOKEN });
    server.boardsStore().set("aide", MAIN_BOARD_KEY, mainEntry());
    const res = await fetch(`${base}/api/queue/projects/aide/test-board/stop`, { method: "POST", headers: AUTH });
    expect(res.status).toBe(200);
...
describe("GET /projects/<name>?startTestBoard=1 (AC-5)", () => {
  test("nobody posted: redirects back to the Deploy tab without starting anything", async () => {
    ...
    const res = await fetch(`${base}/projects/aide?startTestBoard=1`, {
  ...
  test("already running with a url: redirects to the board", async () => {
    const { base, server } = start({ queueToken: TOKEN, boardsIsAlive: () => true });
    server.boardsStore().set(
      "aide",
      MAIN_BOARD_KEY,
  ...
  test("failed: shows boardFailedPage", async () => {
    const { base, server } = start({ queueToken: TOKEN });
    server.boardsStore().set("aide", MAIN_BOARD_KEY, mainEntry({ status: "failed", error: "cannot check out main" }));
    const res = await fetch(`${base}/projects/aide?startTestBoard=1`, {
      headers: { "x-aide-token": TOKEN },
      redirect: "manual",
    });
  ...
  test("still starting: shows the waiting page", async () => {
    const { base, server } = start({ queueToken: TOKEN, boardsIsAlive: () => true });
    server.boardsStore().set("aide", MAIN_BOARD_KEY, mainEntry({ status: "starting", pid: undefined, url: undefined }));
    const res = await fetch(`${base}/projects/aide?startTestBoard=1`, {
      headers: { "x-aide-token": TOKEN },
      redirect: "manual",
    });
