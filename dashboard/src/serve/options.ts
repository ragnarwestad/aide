...
export interface ServerOptions {
   ...
   *  like `testServersSpawn`: no test should actually end the process
   *  running it. */
  selfStopExit?: () => void;
   ...
   *  (spec 388). A test seam, like `testServersSpawn`. */
  testServersIsAlive?: (pid: number) => boolean;
  /** Asks what is listening on one of the test-server ports, so a board
   *  still running is found again after a restart (`test-servers/recover.ts`).
