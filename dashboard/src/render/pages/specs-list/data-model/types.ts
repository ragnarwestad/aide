 ...
 *  wrote them with.
 *
 *  It is deliberately NOT a `SpecTarget`: a target is a spec the queue
...
export const RUN_STEPS = ["analyze", "implement", "archive"];
...
// `RUN_STEPS` directly. Only the phase-line list reads this one.
//
...
export const PHASE_LINES = ["create", ...RUN_STEPS];
...
export interface SpecGroup extends HasNotVerified {
  ...
  /** True when `totalDurationMs` leaned on at least one phase's own
   *  file stamp rather than a queue-measured span for it (spec 410,
   *  REQ-4) — mirrors `costUnmeasured`'s own shape. Computed the same
