// The Steps tab's reload, from script (its meta refresh sits in `<noscript>`
// on a page that has script): every N seconds the page reloads, except that a
// tick is skipped while a dialog is open, so a reason half-typed in Close's
// box is not lost.
