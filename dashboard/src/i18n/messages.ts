...
export const MESSAGES = {
  ...
  "landing.deployServiceOlder": {
    en: "the service is running commit {served}, but the checkout is on {head} — press Deploy again to restart it",
    nb: "tjenesten kjører commit {served}, men det lokale repoet står på {head} — trykk Deploy på nytt for å starte den om igjen",
    es: "el servicio ejecuta el commit {served}, pero el checkout está en {head} — pulsa Deploy de nuevo para reiniciarlo",
    de: "der Dienst führt Commit {served} aus, der Checkout steht aber auf {head} — drücke erneut Deploy, um ihn neu zu starten",
    fr: "le service exécute le commit {served}, mais le checkout est sur {head} — appuyez de nouveau sur Deploy pour le redémarrer",
    resolve: "press Deploy",
  },
  "landing.deployNoAnswer": {
    en: "the service did not answer after the restart — check it on the serving host, then press Deploy again",
    nb: "tjenesten svarte ikke etter omstarten — sjekk den på serveren, og trykk så Deploy på nytt",
    es: "el servicio no respondió tras el reinicio — revísalo en el servidor y pulsa Deploy de nuevo",
    de: "der Dienst hat nach dem Neustart nicht geantwortet — prüfe ihn auf dem Server und drücke dann erneut Deploy",
    fr: "le service n'a pas répondu après le redémarrage — vérifiez-le sur le serveur, puis appuyez de nouveau sur Deploy",
    resolve: "check it on the serving host",
  },
  "landing.deployNoRestart": {
    en: "nothing on this machine restarts the service, so it still runs commit {served} and the checkout is on {head} — restart it by hand",
    nb: "ingenting på denne maskinen starter tjenesten på nytt, så den kjører fortsatt commit {served} mens det lokale repoet står på {head} — start den på nytt for hånd",
    es: "nada en esta máquina reinicia el servicio, así que sigue ejecutando el commit {served} y el checkout está en {head} — reinícialo a mano",
    de: "nichts auf diesem Rechner startet den Dienst neu, er führt also weiter Commit {served} aus, der Checkout steht auf {head} — starte ihn von Hand neu",
    fr: "rien sur cette machine ne redémarre le service, il exécute donc toujours le commit {served} alors que le checkout est sur {head} — redémarrez-le à la main",
