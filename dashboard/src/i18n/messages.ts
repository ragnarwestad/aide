...
export const MESSAGES = {
  ...
  "runner.testsRedImplement": {
    ...
    es: "implement informó de éxito, pero los tests del proyecto están en rojo sobre su resultado — el runner los ejecutó él mismo, y los tests que fallan están en el detalle. — Pulsa {button} de nuevo; el paso solo termina con una ejecución en verde.",
    de: "Implement meldete Erfolg, aber die Tests des Projekts sind bei diesem Ergebnis rot — der Runner hat sie selbst ausgeführt, und die fehlschlagenden Tests stehen im Detail. — Klicke erneut auf {button}; der Schritt endet erst bei einem grünen Lauf.",
    fr: "implement a signalé un succès, mais les tests du projet sont au rouge sur ce résultat — le runner les a lancés lui-même, et les tests en échec sont dans le détail. — Cliquez de nouveau sur {button} ; l'étape ne se termine que sur une exécution au vert.",
    resolve: "Press {button} again; the step ends only on a green run.",
  },
  "runner.testsRedArchive": {
    en: "archive reported success, but the project's tests are red after the merge with main — the runner ran them itself, and the failing tests are in the detail. — Press {button} again; the step ends only on a green run.",
    nb: "arkivering meldte ferdig, men prosjektets tester er røde etter mergen med main — runneren kjørte dem selv, og testene som feiler står i detaljene. — Trykk {button} igjen; steget ender bare på en grønn kjøring.",
    es: "archive informó de éxito, pero los tests del proyecto están en rojo tras el merge con main — el runner los ejecutó él mismo, y los tests que fallan están en el detalle. — Pulsa {button} de nuevo; el paso solo termina con una ejecución en verde.",
    de: "Archive meldete Erfolg, aber die Tests des Projekts sind nach dem Merge mit main rot — der Runner hat sie selbst ausgeführt, und die fehlschlagenden Tests stehen im Detail. — Klicke erneut auf {button}; der Schritt endet erst bei einem grünen Lauf.",
    fr: "archive a signalé un succès, mais les tests du projet sont au rouge après le merge avec main — le runner les a lancés lui-même, et les tests en échec sont dans le détail. — Cliquez de nouveau sur {button} ; l'étape ne se termine que sur une exécution au vert.",
    resolve: "Press {button} again; the step ends only on a green run.",
  },
  ...
  "wordPhase.stopAcceptanceCriteriaUnticked": {
    ...
    es: "no todos los criterios de Aceptación están marcados — márcalos en la pestaña Checks",
    de: "nicht alle Abnahmekriterien sind abgehakt — hake sie im Tab Checks ab",
    fr: "les critères d'Acceptation ne sont pas tous cochés — cochez-les dans l'onglet Checks",
